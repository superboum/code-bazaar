...
(define max-disk-space (- disk-space update-space))
(define (missing-space fstree) (- (cadr fstree) max-disk-space))

; -- folder just big enough for the update
(define (big-enough fstree target)
  (cond
    ((null? (cddr fstree)) disk-space) ; not interested by files
    (#t 
     (apply min (filter 
       (lambda (s) (>= s target)) 
       (cons 
         (cadr fstree) 
         (map (lambda (f) (big-enough f target)) (cddr fstree))))))
))
...
(let* ([visitor (parser lex-stdin (obs-gen '("/" 0) '()))]
       ...
       [ms (missing-space stats)]
       [be (big-enough stats ms)])
  (format #t "~a~%" be))
