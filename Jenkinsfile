...
node {
  stage('Cheatsheet') {
    docker.image('debian').inside("-u root") {
      sh 'apt-get update && apt-get install --no-install-recommends -y git texlive-latex-base texlive-latex-extra lmodern texlive-lang-french python-pigments'
      checkout scm
  ...
  stage('Common Lisp') {
    docker.image('debian').inside("-u root") {
      sh 'apt-get update && apt-get install --no-install-recommends -y git sbcl'
      checkout scm
